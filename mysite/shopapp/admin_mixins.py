...
class ExportAsCSVMixin:
    def export_csv(self, request: HttpRequest, queryset: QuerySet):
        ...
        filed_names = [field.name for field in meta.fields]

        response = HttpResponse(content_type='text/csv')
        response['Context-Disposition'] = f'attachment; filename={meta}-export.csv'
        ...
        for obj in queryset:
            csv_writer.writerow([getattr(obj, field) for field in filed_names])
